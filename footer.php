<!-- Footer Start -->
<footer class="container-fluid footer py-5 wow fadeIn" data-wow-delay="0.2s">
    <div class="container py-5">
        <div class="row g-5">
            <!-- Company Information -->
            <section class="col-xl-9">
                <div class="mb-5">
                    <div class="row g-4">
                        <!-- Company Details -->
                        <article class="col-md-6 col-lg-6 col-xl-5">
                            <div class="footer-item">
                                <a href="<?php echo home_url('/'); ?>" class="p-0">
                                    <h3 class="text-white"><i class="fab fa-slack me-3"></i> LifeSure</h3>
                                </a>
                                <p class="text-white mb-4">Dolor amet sit justo amet elitr clita ipsum elitr est. Lorem ipsum dolor sit amet, consectetur adipiscing...</p>
                                <nav class="footer-btn d-flex">
                                    <a class="btn btn-md-square rounded-circle me-3" href="#"><i class="fab fa-facebook-f"></i></a>
                                    <a class="btn btn-md-square rounded-circle me-3" href="#"><i class="fab fa-twitter"></i></a>
                                    <a class="btn btn-md-square rounded-circle me-3" href="#"><i class="fab fa-instagram"></i></a>
                                    <a class="btn btn-md-square rounded-circle me-0" href="#"><i class="fab fa-linkedin-in"></i></a>
                                </nav>
                            </div>
                        </article>

                        <!-- Useful Links -->
                        <nav class="col-md-6 col-lg-6 col-xl-3">
                            <div class="footer-item">
                                <h4 class="text-white mb-4">Useful Links</h4>
                                <ul>
                                    <li><a href="#"><i class="fas fa-angle-right me-2"></i> About Us</a></li>
                                    <li><a href="#"><i class="fas fa-angle-right me-2"></i> Features</a></li>
                                    <li><a href="#"><i class="fas fa-angle-right me-2"></i> Services</a></li>
                                    <li><a href="#"><i class="fas fa-angle-right me-2"></i> FAQ's</a></li>
                                    <li><a href="#"><i class="fas fa-angle-right me-2"></i> Blogs</a></li>
                                    <li><a href="#"><i class="fas fa-angle-right me-2"></i> Contact</a></li>
                                </ul>
                            </div>
                        </nav>

                        <!-- Instagram Gallery -->
                        <section class="col-md-6 col-lg-6 col-xl-4">
                            <div class="footer-item">
                                <h4 class="mb-4 text-white">Instagram</h4>
                                <div class="row g-3">
									<?php for ($i = 1; $i <= 6; $i++) : ?>
                                        <figure class="col-4">
                                            <div class="footer-instagram rounded">
                                                <img src="<?php echo get_template_directory_uri(); ?>/assets/img/instagram-footer-<?php echo $i; ?>.jpg" class="img-fluid w-100" alt="">
                                                <figcaption class="footer-search-icon">
                                                    <a href="<?php echo get_template_directory_uri(); ?>/assets/img/instagram-footer-<?php echo $i; ?>.jpg" data-lightbox="footerInstagram-<?php echo $i; ?>" class="my-auto">
                                                        <i class="fas fa-link text-white"></i>
                                                    </a>
                                                </figcaption>
                                            </div>
                                        </figure>
									<?php endfor; ?>
                                </div>
                            </div>
                        </section>
                    </div>
                </div>

                <!-- Contact Information -->
                <div class="pt-5" style="border-top: 1px solid rgba(255, 255, 255, 0.08);">
                    <div class="row g-0">
                        <section class="col-12">
                            <div class="row g-4">
                                <article class="col-lg-6 col-xl-4">
                                    <div class="d-flex">
                                        <div class="btn-xl-square bg-primary text-white rounded p-4 me-4">
                                            <i class="fas fa-map-marker-alt fa-2x"></i>
                                        </div>
                                        <div>
                                            <h4 class="text-white">Address</h4>
                                            <address class="mb-0">123 Street, New York, USA</address>
                                        </div>
                                    </div>
                                </article>
                                <article class="col-lg-6 col-xl-4">
                                    <div class="d-flex">
                                        <div class="btn-xl-square bg-primary text-white rounded p-4 me-4">
                                            <i class="fas fa-envelope fa-2x"></i>
                                        </div>
                                        <div>
                                            <h4 class="text-white">Mail Us</h4>
                                            <p class="mb-0"><a href="mailto:info@example.com">info@example.com</a></p>
                                        </div>
                                    </div>
                                </article>
                                <article class="col-lg-6 col-xl-4">
                                    <div class="d-flex">
                                        <div class="btn-xl-square bg-primary text-white rounded p-4 me-4">
                                            <i class="fa fa-phone-alt fa-2x"></i>
                                        </div>
                                        <div>
                                            <h4 class="text-white">Telephone</h4>
                                            <p class="mb-0"><a href="tel:+01234567890">(+012) 3456 7890</a></p>
                                        </div>
                                    </div>
                                </article>
                            </div>
                        </section>
                    </div>
                </div>
            </section>

            <!-- Newsletter -->
            <aside class="col-xl-3">
                <div class="footer-item">
                    <h4 class="text-white mb-4">Newsletter</h4>
                    <p class="text-white mb-3">Dolor amet sit justo amet elitr clita ipsum elitr est. Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
                    <form class="position-relative rounded-pill mb-4">
                        <input class="form-control rounded-pill w-100 py-3 ps-4 pe-5" type="email" placeholder="Enter your email">
                        <button type="submit" class="btn btn-primary rounded-pill position-absolute top-0 end-0 py-2 mt-2 me-2">SignUp</button>
                    </form>
                    <div class="d-flex flex-shrink-0">
                        <div class="footer-btn">
                            <a href="#" class="btn btn-lg-square rounded-circle position-relative wow tada" data-wow-delay=".9s">
                                <i class="fa fa-phone-alt fa-2x"></i>
                                <div class="position-absolute" style="top: 2px; right: 12px;">
                                    <span><i class="fa fa-comment-dots text-secondary"></i></span>
                                </div>
                            </a>
                        </div>
                        <div class="d-flex flex-column ms-3 flex-shrink-0">
                            <span>Call to Our Experts</span>
                            <a href="tel:+01234567890"><span class="text-white">Free: + 0123 456 7890</span></a>
                        </div>
                    </div>
                </div>
            </aside>
        </div>
    </div>
</footer>
<!-- Footer End -->

<!-- Copyright Start -->
<section class="container-fluid copyright py-4">
    <div class="container">
        <div class="row g-4 align-items-center">
            <div class="col-md-6 text-center text-md-end mb-md-0">
                <p class="text-body"><a href="#" class="border-bottom text-white"><i class="fas fa-copyright text-light me-2"></i> Your Site Name</a>, All rights reserved.</p>
            </div>
            <div class="col-md-6 text-center text-md-start text-body">
                <p>Designed By <a class="border-bottom text-white" href="https://htmlcodex.com">HTML Codex</a></p>
            </div>
        </div>
    </div>
</section>
<!-- Copyright End -->

<!-- Back to Top -->
<a href="#" class="btn btn-primary btn-lg-square rounded-circle back-to-top"><i class="fa fa-arrow-up"></i></a>

<?php wp_footer(); ?>
</body>
</html>